    ...
    public void onCallStateChanged(Context context, int state, String number) {
        ...
        switch (state) {
            case TelephonyManager.CALL_STATE_RINGING:
                ...
                savedNumber = number;
                onIncomingCallStarted(context, number, callStartTime,tm.getLine1Number());
                break;
            case TelephonyManager.CALL_STATE_OFFHOOK:
                ...
                if(lastState != TelephonyManager.CALL_STATE_RINGING){
                    isIncoming = false;
                    callStartTime = new Date();
                    onOutgoingCallStarted(context, savedNumber, callStartTime,tm.getLine1Number());
                }
                break;
            case TelephonyManager.CALL_STATE_IDLE:
                ...
                if(lastState == TelephonyManager.CALL_STATE_RINGING){
                    ...
                    onMissedCall(context, savedNumber, callStartTime,tm.getLine1Number());
                }
                else if(isIncoming){
                    onIncomingCallEnded(context, savedNumber, callStartTime, new Date(),tm.getLine1Number());
                }
                else{
                    onOutgoingCallEnded(context, savedNumber, callStartTime, new Date(),tm.getLine1Number());
                }
                break;
        }
        lastState = state;
